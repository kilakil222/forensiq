...
func ParseTemplateInstance(ctx *ParseContext) bool {
	...
	*/
	...
	// Sanity check: an EVTX chunk is 64 KB; more than 65536 arguments means
	// the count field is corrupted. Without this guard the library tries to
	// VirtualAlloc several GB and the Go runtime dies with a fatal OOM.
	if numArguments > 65536 {
		return false
	}
